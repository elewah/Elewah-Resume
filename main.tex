...
\begin{document}
    ...
    \section{Summary}
        ...
        \begin{onecolentry}
    AI and Machine Learning Engineer with over 7 years of experience developing scalable AI and IoT systems. Expertise in LLMs, RAG, and real-time IoT data frameworks. Proven success in designing and implementing innovative solutions that connect AI with practical applications. Skilled in cloud-based platforms, e-commerce systems, and data analytics. Adept at collaborating with cross-functional teams to achieve impactful results.
...
\textbf{Story-to-Movie Recommender Chatbot (RAG-based)}
\begin{highlights}
\item Built a \textbf{RAG-based chatbot} that matches user-movies descriptions to movies using \textbf{semantic search} and \textbf{GPT-3.5 completions}.\href{https://chatbot-story-to-movie.streamlit.app/}{Live demo}
\item Designed a full \textbf{retrieval pipeline}: embedding movie overviews (\textbf{text-embedding-ada-002}) and ranking results via \textbf{cosine similarity}.
\item Applied \textbf{prompt engineering} to convert top search results into \textbf{GPT-ready context} for \textbf{natural-language recommendations}.
\item Utilized \textbf{Pandas}, \textbf{NumPy}, \textbf{OpenAI API},\textbf{tiktoken} and \textbf{Tenacity} 
\item Demonstrated accurate outputs (e.g., recommending \textit{Inception} based on the prompt: "A man is stuck in a dream world where time moves differently").
differently..)
\end{highlights}
\end{twocolentry}
